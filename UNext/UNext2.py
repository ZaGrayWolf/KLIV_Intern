...
def get_pool_output_shape(input_shape, kernel_size, stride, padding=0, dilation=1):
    ...
    # The pooling formula is slightly simpler as dilation usually applies to the *input* region
    # Effectively, it's the same as Conv with D=1 for output shape calculation
    # (H_in + 2*P_H - K_H) / S_H + 1
...
# --- Manual FLOP Calculation Function (Revised) ---
...
def calculate_manual_flops(model, input_shape):
    ...
    # print(f"Starting FLOPs calculation for {type(model).__name__} with input shape {input_shape}")
    ...
    # Input to PatchEmbed3 is the output of encoder3 (after maxpool)
    ...
    for i, block in enumerate(model.block1):
        ...
        # Need to reshape (B, N, hidden_dim) to (B, hidden_dim, H, W) for DWConv
        shape_dwconv_in = (B_pe3, hidden_dim, H_pe3, W_pe3)
        # Corrected: Pass the inner nn.Conv2d module
    ...
    # The spatial shape corresponds to the output of the patch_embed's convolution
    ...
    # Input to PatchEmbed4 is the spatial output from Stage 4 processing
    patch_embed_in_shape = current_shape
    ...
    f_pe_norm, current_shape_flat = flops_norm(current_shape_flat)
    total_flops += f_pe_norm
    # The output of Norm4 is the input to the decoder after reshaping back to spatial
    # No separate skip from Stage 5 bottleneck to decoder typically
    ...
    # The spatial shape corresponds to the output of the patch_embed4's convolution
    current_shape = (B_pe4, C_pe4_out, H_pe4, W_pe4) # (B, C_stage5, H_stage5, W_stage5)
    # print(f"  End Stage 5 (Bottleneck). Current spatial shape: {current_shape}")


    # --- Decoder ---
    # Decoder 1 (Upsample + Concat t4 + Conv -> BN -> ReLU) + DBlock1
    # print("Decoder Stage 1 (Upsample + Concat t4 + Conv -> BN -> ReLU + DBlock1)...")
    # Input shape is current_shape from end of Stage 5 (Bottleneck)
    # Upsample to match t4 spatial shape (shape_dict['t4_spatial_for_skip'])
    skip_shape_t4 = shape_dict['t4_spatial_for_skip'] # This is the spatial shape *after* patch embed conv in Stage 4
    B_dec1, C_dec1_in, H_dec1_in, W_dec1_in = current_shape
    _, C_skip_t4, H_skip_t4, W_skip_t4 = skip_shape_t4
    upsample_output_size_dec1 = (H_skip_t4, W_skip_t4)
    f_upsample, current_shape = flops_upsample_bilinear(current_shape, upsample_output_size_dec1)
    total_flops += f_upsample # current_shape is now (B, C_dec1_in, H_skip_t4, W_skip_t4)

    # Concatenate with t4 skip connection
    # The skip connection `skip_shape_t4` has shape (B, C_t4, H_skip_t4, W_skip_t4)
    # Input channel count for the conv is C_dec1_in (from upsample) + C_t4 (from skip)
    current_shape_cat_in = (B_dec1, C_dec1_in + C_skip_t4, H_skip_t4, W_skip_t4)
    # Concatenation is ~0 FLOPs

    # decoder1 Conv
    f, current_shape = flops_conv2d(model.decoder1, current_shape_cat_in)
    total_flops += f
    # dbn1
    f, current_shape = flops_norm(current_shape)
    total_flops += f
    # relu
    f, current_shape = flops_activation(current_shape)
    total_flops += f
    B_dec1_out, C_dec1_out, H_dec1_out, W_dec1_out = current_shape # Spatial shape after decoder1 Conv+BN+ReLU

    # DBlock 1 (shiftedBlock) - operates on spatial feature map reshaped to sequence
    # print(f"  DBlock 1 ({len(model.dblock1)} blocks)...")
    # Reshape spatial (B, C_dec1_out, H_dec1_out, W_dec1_out) to sequence (B, N, C)
    N_db1 = H_dec1_out * W_dec1_out
    current_shape_block = (B_dec1_out, N_db1, C_dec1_out)

    for i, block in enumerate(model.dblock1):
        # print(f"    DBlock 1.{i}...")
        shape_before_block = current_shape_block # (B, N, C)
        dim = shape_before_block[-1] # C
        mlp_ratio = model.mlp_ratios[0] # DBlock1 uses mlp_ratios[0]
        hidden_dim = int(dim * mlp_ratio)
        ...
        f_fc1, shape_after_fc1 = flops_linear(block.mlp.fc1, shape_after_norm)
        total_flops += f_fc1
        shape_dwconv_in = (B_dec1_out, hidden_dim, H_dec1_out, W_dec1_out) # Spatial shape for DWConv
         # Corrected: Pass the inner nn.Conv2d module
        f_dwc, shape_after_dwc = flops_dwconv2d(block.mlp.dwconv.dwconv, shape_dwconv_in)
        total_flops += f_dwc
        shape_after_dwc_flat = (B_dec1_out, N_db1, hidden_dim)
        f_act, shape_after_act = flops_activation(shape_after_dwc_flat)
        total_flops += f_act
        f_fc2, shape_after_fc2 = flops_linear(block.mlp.fc2, shape_after_act)
        total_flops += f_fc2
    ...
    # DNorm 3 after DBlock1
    # print("  DNorm 3 after DBlock 1...")
    f, current_shape_flat = flops_norm(current_shape_block) # model.dnorm3 operates on (B, N, C)
    total_flops += f
    # Reshape back to spatial (B, C_dec1_out, H_dec1_out, W_dec1_out)
    current_shape = (B_dec1_out, C_dec1_out, H_dec1_out, W_dec1_out)
    # print(f"  End DBlock 1. Current spatial shape: {current_shape}")


    # Decoder 2 (Upsample + Concat t3 + Conv -> BN -> ReLU) + DBlock2
    # print("Decoder Stage 2 (Upsample + Concat t3 + Conv -> BN -> ReLU + DBlock2)...")
    # Input shape is current_shape from end of DBlock 1 processing
    # Upsample to match t3 spatial shape (shape_dict['t3'])
    skip_shape_t3 = shape_dict['t3'] # Spatial shape after encoder Stage 3 Conv+BN+ReLU
    B_dec2, C_dec2_in, H_dec2_in, W_dec2_in = current_shape
    _, C_skip_t3, H_skip_t3, W_skip_t3 = skip_shape_t3
    upsample_output_size_dec2 = (H_skip_t3, W_skip_t3)
    f_upsample, current_shape = flops_upsample_bilinear(current_shape, upsample_output_size_dec2)
    total_flops += f_upsample # current_shape is now (B, C_dec2_in, H_skip_t3, W_skip_t3)

    # Concatenate with t3 skip connection
    # The skip connection `skip_shape_t3` has shape (B, C_t3, H_skip_t3, W_skip_t3)
    current_shape_cat_in = (B_dec2, C_dec2_in + C_skip_t3, H_skip_t3, W_skip_t3)

    # decoder2 Conv
    f, current_shape = flops_conv2d(model.decoder2, current_shape_cat_in)
    total_flops += f
    # dbn2
    f, current_shape = flops_norm(current_shape)
    total_flops += f
    ...
    f, current_shape = flops_activation(current_shape)
    total_flops += f
    B_dec2_out, C_dec2_out, H_dec2_out, W_dec2_out = current_shape # Spatial shape after decoder2 Conv+BN+ReLU

    # DBlock 2 (shiftedBlock) - operates on spatial feature map reshaped to sequence
    # print(f"  DBlock 2 ({len(model.dblock2)} blocks)...")
    N_db2 = H_dec2_out * W_dec2_out
    current_shape_block = (B_dec2_out, N_db2, C_dec2_out)

    for i, block in enumerate(model.dblock2):
        # print(f"    DBlock 2.{i}...")
        shape_before_block = current_shape_block # (B, N, C)
        dim = shape_before_block[-1] # C
        mlp_ratio = model.mlp_ratios[1] # DBlock2 uses mlp_ratios[1]
        hidden_dim = int(dim * mlp_ratio)
        ...
        f_fc1, shape_after_fc1 = flops_linear(block.mlp.fc1, shape_after_norm)
        total_flops += f_fc1
        shape_dwconv_in = (B_dec2_out, hidden_dim, H_dec2_out, W_dec2_out) # Spatial shape for DWConv
         # Corrected: Pass the inner nn.Conv2d module
        f_dwc, shape_after_dwc = flops_dwconv2d(block.mlp.dwconv.dwconv, shape_dwconv_in)
        total_flops += f_dwc
        shape_after_dwc_flat = (B_dec2_out, N_db2, hidden_dim)
        f_act, shape_after_act = flops_activation(shape_after_dwc_flat)
        total_flops += f_act
        f_fc2, shape_after_fc2 = flops_linear(block.mlp.fc2, shape_after_act)
        total_flops += f_fc2
    ...
    # DNorm 4 after DBlock2
    # print("  DNorm 4 after DBlock 2...")
    f, current_shape_flat = flops_norm(current_shape_block) # model.dnorm4 operates on (B, N, C)
    total_flops += f
    # Reshape back to spatial (B, C_dec2_out, H_dec2_out, W_dec2_out)
    current_shape = (B_dec2_out, C_dec2_out, H_dec2_out, W_dec2_out)
    # print(f"  End DBlock 2. Current spatial shape: {current_shape}")


    # Decoder 3 (Upsample + Concat t2 + Conv -> BN -> ReLU)
    # print("Decoder Stage 3 (Upsample + Concat t2 + Conv -> BN -> ReLU)...")
    # Input shape is current_shape from end of DBlock 2 processing
    # Upsample to match t2 spatial shape (shape_dict['t2'])
    skip_shape_t2 = shape_dict['t2'] # Spatial shape after encoder Stage 2 Conv+BN+ReLU
    B_dec3, C_dec3_in, H_dec3_in, W_dec3_in = current_shape
    _, C_skip_t2, H_skip_t2, W_skip_t2 = skip_shape_t2
    upsample_output_size_dec3 = (H_skip_t2, W_skip_t2)
    f_upsample, current_shape = flops_upsample_bilinear(current_shape, upsample_output_size_dec3)
    total_flops += f_upsample # current_shape is now (B, C_dec3_in, H_skip_t2, W_skip_t2)

    # Concatenate with t2 skip connection
    current_shape_cat_in = (B_dec3, C_dec3_in + C_skip_t2, H_skip_t2, W_skip_t2)

    # decoder3 Conv
    f, current_shape = flops_conv2d(model.decoder3, current_shape_cat_in)
    total_flops += f
    # dbn3
    f, current_shape = flops_norm(current_shape)
    total_flops += f
    ...
    # Decoder 4 (Upsample + Concat t1 + Conv -> BN -> ReLU)
    # print("Decoder Stage 4 (Upsample + Concat t1 + Conv -> BN -> ReLU)...")
    # Input shape is current_shape from end of Decoder 3 processing
    # Upsample to match t1 spatial shape (shape_dict['t1'])
    skip_shape_t1 = shape_dict['t1'] # Spatial shape after encoder Stage 1 Conv+BN+ReLU
    B_dec4, C_dec4_in, H_dec4_in, W_dec4_in = current_shape
    _, C_skip_t1, H_skip_t1, W_skip_t1 = skip_shape_t1
    upsample_output_size_dec4 = (H_skip_t1, W_skip_t1)
    f_upsample, current_shape = flops_upsample_bilinear(current_shape, upsample_output_size_dec4)
    total_flops += f_upsample # current_shape is now (B, C_dec4_in, H_skip_t1, W_skip_t1)

    # Concatenate with t1 skip connection
    current_shape_cat_in = (B_dec4, C_dec4_in + C_skip_t1, H_skip_t1, W_skip_t1)

    # decoder4 Conv
    f, current_shape = flops_conv2d(model.decoder4, current_shape_cat_in)
    total_flops += f
    # dbn4
    f, current_shape = flops_norm(current_shape)
    total_flops += f
    ...
    # Decoder 5 (Upsample + Conv -> BN -> ReLU) - Upsample to original input size
    # print("Decoder Stage 5 (Upsample + Conv -> BN -> ReLU)...")
    # Input shape is current_shape from end of Decoder 4 processing
    # Upsample to match original input image spatial shape
    B_dec5, C_dec5_in, H_dec5_in, W_dec5_in = current_shape
    H_orig, W_orig = input_shape[2:]
    upsample_output_size_dec5 = (H_orig, W_orig)
    f_upsample, current_shape = flops_upsample_bilinear(current_shape, upsample_output_size_dec5)
    total_flops += f_upsample # current_shape is now (B, C_dec5_in, H_orig, W_orig)

    # decoder5 Conv
    f, current_shape = flops_conv2d(model.decoder5, current_shape)
    total_flops += f
    # dbn5
    # Check if dbn5 exists before calculating flops (UNext might not have it, UNext_S does)
    if hasattr(model, 'dbn5'): # Assumes UNext_S has dbn5, UNext might not based on specific variant
         f, current_shape = flops_norm(current_shape)
         total_flops += f
    # relu
    f, current_shape = flops_activation(current_shape)
    ...
    total_flops += f
    # print(f"  End Final Conv. Output shape: {current_shape}")

    # Total FLOPs calculation finished.
    return total_flops
...
if __name__ == "__main__":
    ...
    # IMG_SIZE is usually the square size, but UNext can handle variable input sizes.
    # We'll use the input shape dimensions directly for calculations.
    NUM_CLASSES = 4 # Example: 4 classes
    BATCH_SIZE = 1 # FLOPs are usually calculated for B=1

    # Input shape
    # Using a common training/evaluation resolution for segmentation models
    input_shape = (BATCH_SIZE, 3, 640, 360) # B, C, H, W

    # Define depths and mlp_ratios based on typical UNext/UNext-S configurations
    # UNext default: depths=[2, 2], mlp_ratios=[4, 4] (example)
    # UNext-S default: depths=[1, 1], mlp_ratios=[4, 4] (example)
    # Adjust these based on the specific UNext/UNext-S variant you are interested in
    unext_depths = [1, 1]
    unext_mlp_ratios = [4., 4.] # Common MLP expansion ratio

    unext_s_depths = [1, 1]
    unext_s_mlp_ratios = [4., 4.] # Common MLP expansion ratio


    # --- UNext ---
    print("Calculating for UNext...")
    try:
        # Instantiate the simulated UNext model
        ...
        # Count parameters
        unext_params = count_parameters(unext_model)

        # Calculate FLOPs
        unext_flops = calculate_manual_flops(unext_model, input_shape)


        print(f"UNext Model ({unext_depths=}, {unext_mlp_ratios=}):")
        print(f"  Input Shape: {input_shape}")
        print(f"  Parameters: {unext_params / 1e6:.2f} M")
        print(f"  Approx. FLOPs: {unext_flops / 1e9:.2f} GFLOPs")

    except Exception as e:
         print(f"An error occurred during UNext calculation: {e}")
         import traceback
         traceback.print_exc()
    print("-" * 30)
